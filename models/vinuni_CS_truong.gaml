...
experiment Sobol type: batch until:(current_date.hour = 23 and current_date.minute = 55) parallel: 10 {
	...
	parameter "Implement a policy prohibiting gasoline cars from parking in active_CS" var: policy_prohibit_parking category: "Policies" min:0 max:1;
	...
	method sobol outputs:["avg_statisfied_day"] sample:100 report:"Results/sobol.txt" results:"Results/sobol_raw.csv";
}
...
experiment replication_analysis type: batch until: (current_date.hour = 23 and current_date.minute = 55)  
	...
	parameter "Implement a policy prohibiting gasoline cars from parking in active_CS" var: policy_prohibit_parking category: "Policies" min:0 max:1;
	parameter "Implement a policy forcing EVs to move to inactive parking slot when fully charged" var: policy_force_moving category: "Policies" min:0 max:1;
	method stochanalyse outputs:["avg_statisfied_day","monthly_energy_consumption"] report:"Results/stochanalysis.txt" results:"Results/stochanalysis_raw.csv" sample:50;
} 
experiment indicator1_exploration type: batch until: (cycle=287) repeat: 10  {
...
experiment alter_1_indi_1_effectiveness type: gui {
	...
	init {
		...
		create simulation with: [nb_activeCS_Cparking::7, nb_activeCS_Jparking::10];
...
//	reflex save_result_satisfied_percent {
//		save [nb_gasoline, nb_electrical, current_date, percentage_statisfied] 
//	   		//to: "Results/percentage_statisfied" +string(nb_gasoline)+"_"+string(nb_electrical)+".csv" format:"csv" rewrite: (cycle = 0) ? true : false;
//	   		to: "Results/percentage_statisfied.csv"  format:"csv" rewrite: (cycle = 0) ? true : false;
//	}
...
//	reflex save_result_energy {
//		save [nb_gasoline, nb_electrical, current_date, energy_consumption, total_energy_EVs] 
//	   		to: "Results/instant_energy.csv"  format:"csv" rewrite: (cycle = 0) ? true : false;
//	}
...
//		display time_to_charge refresh: every(12 #cycle) {
...
//				data "Not Charged & Unsatisfied" accumulate_values: true value: nbEV_uncharged_unsatisfied color: #red;
//			}
//		}
...
//		}
		...
		display Revenue_Profit type: 2d  refresh: (current_date.hour = 23 and current_date.minute = 55){ 
			chart "Monthly Revenue & Profit (in VND)" type: series memorize: false x_label: "Day"{
...
//		
		display monthly_energy_consumption type: 2d refresh: (current_date.hour = 23 and current_date.minute = 55) {
			chart "Monthly Electric Energy Consumption (kWh)" type: series memorize: false {
				data "monthly_energy_consumption (kWh)" value: monthly_energy_consumption color: #black marker: true style: line;
			}
		} 
//		display daily_energy_consumption type: 2d refresh: (current_date.hour = 23 and current_date.minute = 40)  {
//			chart "Daily Energy Consumption (kWh)" type: series memorize: false {
//				data "total_energy_EVs (kWh)" value:total_energy_EVs color: #black marker: true style: line;
//			}
//		}
