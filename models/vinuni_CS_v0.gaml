...
species car_electrical parent: car {
	...
	bool priority_destination <- flip(0.7) ? true : false;
	int num_checkSlot <- 0;

	
	int SoC <- 10+rnd(80);  //battery level
	string EV_model; //type of EV
	float charging_rate; // charging rate of EV
	list<string> EV_models_at_vinuni <- ["VFe34", "VF8", "VF9"];
	map<string, float> model_charging_rate <- ["VFe34":: 100/46, "VF8":: 100/94, "VF9":: 100/134];
	...
	init {
		...
		charging_rate <- model_charging_rate[EV_model];
	}
...
//	reflex try_to_charge when: moving_obj = "working" {
